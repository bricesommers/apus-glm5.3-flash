/*
 * tests/m2/test_tok.c — GLM-5.3-Flash tokenizer conformance vs Python-
 * generated goldens (HF `tokenizers` on reference/tokenizer.json).
 *
...
#define GLM_N_TOKENS 154856  /* 154,820 BPE + 36 added (ids 154,820..154,855) */
#define GLM_N_ADDED 36
...
int main(void) {
    ...
    CHECK(tok_n_tokens(t) == GLM_N_TOKENS, "n_tokens=%d", tok_n_tokens(t));
    ...
    /* ---- specials coverage ---- */
    {
        ...
        if (sb) {
            unsigned char covered[(GLM_N_TOKENS + 7) / 8];
            ...
            size_t n_cov = 0;
            for (size_t i = 0; i < GLM_N_TOKENS; i++)
                if (covered[i >> 3] & (1u << (i & 7))) n_cov++;
            CHECK(n_cov == nrec, "specials: %zu records but %zu covered", nrec, n_cov);
            CHECK(n_cov == GLM_N_ADDED, "specials: only %zu/%d added ids covered",
                  n_cov, GLM_N_ADDED);
    ...
    /* ---- optional exhaustive codepoint probes ---- */
    {
        ...
        if (cb) {
            CHECK(clen >= 8 && rd_u32(cb) == 0xC0DE0002u, "codepoints: bad magic");
    ...
    /* ---- edge cases ---- */
    {
        ...
        free(d);
        /* generation-config eos set must be added-token ids */
        CHECK(tok_token_to_id(t, "<|endoftext|>") == 154820, "eos 154820");
        CHECK(tok_token_to_id(t, "<|user|>") == 154827, "eos 154827");
        CHECK(tok_token_to_id(t, "<|observation|>") == 154829, "eos 154829");
        CHECK(tok_token_to_id(t, "[gMASK]") == 154822, "gMASK id");
        CHECK(tok_token_to_id(t, "<sop>") == 154824, "sop id");
        CHECK(tok_token_to_id(t, "<think>") == 154841, "think id");
    }
