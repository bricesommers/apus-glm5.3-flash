...
/* Memory-pressure footprint in bytes for the GLM gcache's RSS guard and
 * speculative admission control (P2): on macOS, task_info(TASK_VM_INFO)'s
 * phys_footprint — the metric the kernel's compressor/reclaim actually
 * answers to (resident_size UNDER-reports under compression: a 32.5 GB
 * footprint measured by sample(1) read as < 26 GiB resident_size, so the
 * old guard never fired while the machine thrashed). Other platforms:
 * apus_rss_bytes. 0 on failure (callers treat 0 as "no pressure"). The
 * V4 engine keeps apus_rss_bytes (its battery is pinned). */
uint64_t apus_pressure_bytes(void);
...
uint64_t apus_pressure_bytes(void) {
    ...
    return apus_rss_bytes();    /* fallback */
#else
    return apus_rss_bytes();
#endif
}
